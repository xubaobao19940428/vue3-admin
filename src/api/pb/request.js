...
// Todo 统一请求路径前缀
const BASE = process.env.VUE_APP_API + 'back/proxystream'
 ...
 */
function request (serviceName, interfaceName, requestBody, responseType) {
    ...
    let userInfo = localStorage._adminUserInfo ? JSON.parse(localStorage._adminUserInfo) : null
    let token = []
    if (userInfo) {
    ...
    return httpService({
        ...
        if (data.res.ret.errcode === 1) {
            ...
            Message.warning(`ErrorMsg: ${data.res.ret.msg ? data.res.ret.msg : data.res.ret.errname}, ErrorCode: ${data.res.ret.errcode}`)
        }
        return data.res
    }, (err) => {
        if (err.message) {
