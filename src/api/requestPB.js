    ...
    switch (response.data.ret.errcode) {
        case 3101:
            if (localStorage.getItem('adminToken')) {
                ElMessage.warning(`ErrorMsg: ${response.data.ret.msg ? response.data.ret.msg : response.data.ret.errname}, ErrorCode: ${response.data.ret.errcode}`)
            }
            localStorage.clear()
            router.push({
                name: 'login'
...
function request (serviceName, interfaceName, requestBody, responseType, noTips) {
    ...
    // console.log('req',transformResponseFactory(requestBody))
    const reqData = {
